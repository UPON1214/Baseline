...
class MvDEC(object):
    ...
    '''def new_fit(self, arg, x, y, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        while True:
            ...
            # tmp[0] 通常是 model.compile 后的总 Loss
        ...
        # y_q = y_q/len(x)
        y_mean_pred = y_q.argmax(1)
        return y_pred, y_mean_pred, y_softlabels, MVKLLoss'''
    ...
    def new_fit(self, arg, x, y, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        支持：
            - 多视图 KL 聚类损失
            - 多视图重构损失 (MSE)
            - MVKLLoss 记录总损失，用于绘制平滑曲线
        """
        print('Begin clustering:', '-' * 60)

        # Step 1: 初始化 cluster centers
        input_dic = {f'input{v + 1}': x[v] for v in range(len(x))}
        features = self.encoder.predict(input_dic)

        kmeans = KMeans(n_clusters=self.n_clusters, n_init=100)
        y_pred_init = [kmeans.fit_predict(features[v]) for v in range(len(x))]
        center = [kmeans.cluster_centers_ for _ in range(len(x))]

        # 将 KMeans 中心赋值给聚类层
        for v in range(len(x)):
            self.model.get_layer(name='clustering' + str(v + 1)).set_weights([center[v]])

        # Step 2: deep clustering
        index_array = np.arange(x[0].shape[0])
        index = 0
        ite = 0
        update_interval = UpdateCoo
        MVKLLoss = []

        while ite < int(maxiter):
            # batch 索引
            start = index * batch_size
            end = min((index + 1) * batch_size, x[0].shape[0])
            idx = index_array[start:end]
            x_batch = [x[v][idx] for v in range(len(x))]

            # 构建 target
            # 1) 聚类 soft label
            features_batch = [features[v][idx] for v in range(len(x))]
            p = [self.target_distribution(self.new_P(features_batch[v], center[v]))
                 for v in range(len(x))]

            # 2) 构建 yout 字典，key 必须和模型输出名字一致
            y_batch = {}
            for v in range(len(x)):
                y_batch[f'clustering{v + 1}'] = p[v]  # KL loss
                y_batch[f'v{v + 1}_decoder_0'] = x_batch[v]  # MSE reconstruction

            # train on batch
            tmp = self.train_on_batch(
                xin={f'input{v + 1}': x_batch[v] for v in range(len(x))},
                yout=y_batch
            )
            MVKLLoss.append(tmp[0])  # 只记录总 loss

            # 更新 index 和 iteration
            index = index + 1 if (index + 1) * batch_size <= x[0].shape[0] else 0
            ite += 1

            # 每 update_interval 打印状态
            if ite % update_interval == 0:
                print(f"Iteration {ite}/{int(maxiter)} - Loss: {MVKLLoss[-1]:.4f}")

        # Step 3: 保存模型
        os.makedirs(save_dir, exist_ok=True)
        self.model.save_weights(os.path.join(save_dir, 'model_final.h5'))
        print('Clustering finished. Model saved to', save_dir)

        # Step 4: 预测最终标签
        features = self.encoder.predict(input_dic)
        Q_and_X = self.model.predict(input_dic)
        y_pred = [Q_and_X[v * 2].argmax(1) for v in range(len(x))]  # 每个 view
        y_softlabels = [Q_and_X[v * 2] for v in range(len(x))]

        # 多 view soft label 平均
        y_q = sum([Q_and_X[v * 2] for v in range(len(x))])
        y_mean_pred = y_q.argmax(1)
