...
def train(args):
    ...
    for i in ['DIMVC', 'TEST']:
        ...
        if i == 'DIMVC':
            ...
            y_pred, y_mean_pred, y_softlabels, loss_history = model.new_fit(arg=args, x=x_t, y=y_t,
                                                                            maxiter=args.maxiter,
                                                                            batch_size=args.batch_size,
                                                                            UpdateCoo=args.UpdateCoo,
                                                                            save_dir=args.save_dir)
            # 在此处调用绘图函数
            # 调用修改后的绘图函数，设置 window=50 左右效果较好
            plot_loss_curve(loss_history, args.save_dir, f"DIMVC_Loss_MR_{args.missrate}", window=50)
        # ...
        else:
...
'''def plot_loss_curve(losses, save_dir, filename):
    """绘制损失收敛曲线"""
    os.makedirs(save_dir, exist_ok=True)

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(range(len(losses)), losses, color='blue', linewidth=2, label='Loss')
    ax.set_xlabel('Epochs', fontsize=20, weight='bold')
    ax.set_ylabel('Loss', fontsize=20, weight='bold')
    ...
    latest_path = os.path.join(save_dir, filename + "_latest.png")
    fig.savefig(latest_path, dpi=300, bbox_inches='tight')
    plt.show()
    #plt.close(fig)  # 避免重复显示
    print(f"Saved loss curve to {save_path} and {latest_path}")'''


'''def plot_loss_curve(losses, save_dir, filename, window=30):
    """绘制平滑后的损失收敛曲线"""
    import numpy as np
    import matplotlib.pyplot as plt
    import os
    ...
    if len(losses_arr) > window:
        ...
        indices = range(window-1, len(losses_arr))
    else:
        smoothed = losses_arr
        indices = range(len(losses_arr))

    fig, ax = plt.subplots(figsize=(12,6))

    # 原始 loss (浅色线)
    #ax.plot(range(len(losses_arr)), losses_arr, color='blue', alpha=0.15, linewidth=1, label='Raw Loss')
    # 平滑 loss (主线)
    ax.plot(indices, smoothed, color='blue', linewidth=2, label='Smoothed Loss')

    # 美化
    ax.set_xlabel('Epochs', fontsize=16, weight='bold')
...
def plot_loss_curve(losses, save_dir, filename, window=30):
    ...
    # 美化
    # ===== 只放大坐标轴标签 =====
    ax.set_xlabel('Epochs', fontsize=28, weight='bold')
    ax.set_ylabel('Loss', fontsize=28, weight='bold')
    ...
    # ===== 只加粗外框 =====
    for spine in ax.spines.values():
        spine.set_linewidth(3)   # 只改外框粗细


    # 去掉背景网格（删除或注释掉这行）
    # ax.grid(True, linestyle='--', alpha=0.5)  # 原来的网格设置
