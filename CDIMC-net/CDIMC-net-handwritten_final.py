...
import matplotlib
matplotlib.use('TkAgg')  # 确保在 Windows 上弹出窗口
import matplotlib.pyplot as plt
plt.ion()  # 开启交互模式
...
def get_kNNgraph2(data,K_num):
    ...
    graph = np.zeros((num_sample,num_sample),dtype = int)
...
def pretrain_ae(model):
    ...
    '''
    ae_loss_curve = []  # <<< 新增：记录每个 epoch 的 loss
    print(model)
    ...
    for epoch in range(args.aeproches):
        total_loss = 0.
        for batch_idx in range(int(np.ceil(X0.shape[0]/args.batch_size))):
        ...
        print(f"ae_epoch {epoch} loss={epoch_loss:.8f}")
        ae_loss_curve.append(epoch_loss)
        torch.save(model.state_dict(), args.pretrain_path)

    print(f"model saved to {args.pretrain_path}.")

    # ================== 绘制 AE 预训练损失收敛曲线 ==================
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(range(len(ae_loss_curve)), ae_loss_curve, label='AE Loss', color='blue', linewidth=2)
    ax.set_xlabel('Epochs')
    ax.set_ylabel('Loss')
    ax.set_title('AE Pretraining Convergence')
    ax.legend()
    plt.tight_layout()

    # 保存图片
    os.makedirs("loss_plots", exist_ok=True)
    fig.savefig("loss_plots/ae_pretrain_loss_latest.png", dpi=300, bbox_inches='tight')
...
def train_idec():
    ...
    y_pred_last = np.copy(y_pred)
    kl_loss_curve = []  # <<< 新增：记录每个 epoch 的 KL 总损失

    for epoch in range(int(args.maxiter)):
        total_loss_KL_epoch = 0
        index_array = np.arange(X0.shape[0])
        if args.AE_shuffle:
            np.random.shuffle(index_array)
        ...
        for KL_epoach in range(args.maxKL_epoach):
            for batch_idx in range(int(np.ceil(X0.shape[0] / args.batch_size))):
                ...
                x4 = X4[idx].to(device)
                we = WE[idx].to(device)
                y_pred_sub = torch.tensor(y_pred[idx]).to(device)
                sample_weight_sub = sample_weight[idx].to(device)

                optimizer.zero_grad()
                hidden, kl_loss, vz0, vz1, vz2, vz3, vz4 = model(
                    x0, x1, x2, x3, x4, we, y_pred_sub, cluster_layer, sample_weight_sub
                )

                fusion_loss = kl_loss  # 这里只记录 KL 损失
                total_loss_KL_epoch += fusion_loss.item()
                fusion_loss.backward()
                optimizer.step()

        # 平均 KL 损失
        total_loss_KL_epoch /= (args.maxKL_epoach * int(np.ceil(X0.shape[0] / args.batch_size)))
        kl_loss_curve.append(total_loss_KL_epoch)

        # 更新标签
        y_pred, prelosses = model.update_label(X0, X1, X2, X3, X4, WE, cluster_layer)
        y_pred = y_pred.cpu().numpy()

        print(f"Epoch {epoch}: KL Loss={total_loss_KL_epoch:.6f}")

    # ================== 绘制 KL 损失曲线 ==================
    import time
    import os

    losses = kl_loss_curve
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(range(len(losses)), losses, color='blue', linewidth=2, label='KL Loss')
    ...
    save_path = f"{save_dir}/kl_loss_{timestamp}.png"
    fig.savefig(save_path, dpi=300, bbox_inches='tight')
    latest_path = f"{save_dir}/kl_loss_latest.png"
    fig.savefig(latest_path, dpi=300, bbox_inches='tight')

    fig.canvas.draw()
    plt.show(block=True)  # 阻塞显示窗口
