...
class Apadc():
    ...
    def train(self, config, logger, x1_train, x2_train, x3_train, x4_train, Y_list, mask, optimizer, device):
        """Training the model.
            Args:
        ...
        # Get complete data for training
           ...
           # print('yes')
        flag_1 = (torch.LongTensor([1, 1, 1, 1]).to(device) == mask).int()
       # print(flag_1)
        ...
        for epoch in range(config['training']['epoch'] + 1):
            ...
            loss_all, loss_rec1, loss_rec2, loss_rec3, loss_rec4, loss_mmi, loss_mmd = 0, 0, 0, 0, 0, 0, 0
            for batch_x1, batch_x2, batch_x3, batch_x4, x1_index, x2_index, x3_index, x4_index, batch_No in next_batch(X1, X2, X3, X4, X1_index, X2_index, X3_index, X4_index, config['training']['batch_size']):
                if len(batch_x1) == 1:
                    continue
                index_both = x1_index + x2_index + x3_index + x4_index == 4                     # C in indicator matrix A of complete multi-view data
                index_peculiar1 = (x1_index + x1_index + x2_index + x3_index + x4_index == 3)# & (x1_index == 1)     # I^1 in indicator matrix A of incomplete multi-view data
                index_peculiar2 = (x1_index + x2_index + x2_index + x3_index + x4_index == 3) & (x2_index == 1)    # I^2 in indicator matrix A of incomplete multi-view data
                index_peculiar3 = (x1_index + x2_index + x3_index + x3_index + x4_index == 3) & (x3_index == 1)
                index_peculiar4 = (x1_index + x2_index + x3_index + x4_index + x4_index == 3) & (x4_index == 1)
                ...
                rec_loss = (recon1 + recon2 + recon3 + recon4)
                ...
                Z = torch.add(torch.add(z_view1_both * a1, z_view2_both * a2),
                              torch.add(z_view3_both * a3, z_view4_both * a4))
                # mutual information losses \sum L_MMI^v (Z_C, Z_I^v)
                mmi_loss = MMI(z_view1_both, Z) + MMI(z_view2_both, Z) + MMI(z_view3_both, Z) + MMI(z_view4_both, Z)

                view1 = torch.cat([z_view1_both, z_view1_peculiar, z_view2_peculiar, z_view3_peculiar, z_view4_peculiar], dim=0)
                view2 = torch.cat([z_view2_both, z_view1_peculiar, z_view2_peculiar, z_view3_peculiar, z_view4_peculiar], dim=0)
                view3 = torch.cat([z_view3_both, z_view1_peculiar, z_view2_peculiar, z_view3_peculiar, z_view4_peculiar], dim=0)
                view4 = torch.cat([z_view4_both, z_view1_peculiar, z_view2_peculiar, z_view3_peculiar, z_view4_peculiar], dim=0)
            ...
            if (epoch) % config['print_num'] == 0:
                ...
                print(output)
                # evalution
                scores = self.evaluation(config, logger, mask, x1_train, x2_train, x3_train, x4_train, Y_list, device)
    ...
    def evaluation(self, config, logger, mask, x1_train, x2_train, x3_train, x4_train, Y_list, device):
        with torch.no_grad():
            self.autoencoder1.eval(), self.autoencoder2.eval(), self.autoencoder3.eval(), self.autoencoder4.eval()
            
            #flag = mask[:, 0] + mask[:, 1] == 2           # complete multi-view data
            flag = (mask.sum(dim=1) == 4)
            ...
            if flag.any():  # 如果有完全多视图数据
                ...
                common_view4 = self.autoencoder4.encoder(common_view4.to(device))
            ...
            view4_exist = self.autoencoder4.encoder(view4_exist.to(device))
            ...
            common = torch.add(add_common, common_view4).div(4)
            ...
            print(latent_fusion.shape)  # (428, 32)
            print(Y_list[:, 0].shape)  # (428,)
            scores, _ = evaluation.clustering([latent_fusion], Y_list[:, 0])
            # print("\033[2;29m" + 'Common features ' + str(scores) + "\033[0m")
            self.autoencoder1.train(), self.autoencoder2.train(), self.autoencoder3.train(), self.autoencoder4.train()
