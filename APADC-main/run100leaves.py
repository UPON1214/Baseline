...
def main(MR=[0.3]):
    ...
    x1 = X_list[0].reshape(400, 512)
    x2 = X_list[1].reshape(400, 59)
    x3 = X_list[2].reshape(400, 864)
    x4 = X_list[3].reshape(400, 254)
    ...
    for missingrate in MR:
        ...
        for data_seed in range(1, args.test_time + 1):
            ...
            mask = get_mask(4, x1_train_raw.shape[0], config['training']['missing_rate'])
            # mask the data
            ...
            # Training
            acc, nmi, ari = APADC.train(config, logger, x1_train, x2_train, x3_train, x4_train, Y_list, mask, optimizer, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=int, default=str(5), help='dataset id')  # data index str(0)
