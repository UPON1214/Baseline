...
def main(MR=[0.3]):
    ...
    x1 = X_list[0].reshape(1440, 1024)
    x2 = X_list[1].reshape(1440, 1024)
    x3 = X_list[2].reshape(1440, 324)
    ...
    for missingrate in MR:
        ...
        for data_seed in range(1, args.test_time + 1):
            ...
            mask = get_mask(3, x1_train_raw.shape[0], config['training']['missing_rate'])
            # mask the data
            ...
            optimizer = torch.optim.Adam(
                itertools.chain(APADC.autoencoder1.parameters(), APADC.autoencoder2.parameters(), APADC.autoencoder3.parameters()),
            ...
            # Training
            acc, nmi, ari = APADC.train(config, logger, x1_train, x2_train, x3_train, Y_list, mask, optimizer, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=int, default=str(6), help='dataset id')  # data index str(0)
