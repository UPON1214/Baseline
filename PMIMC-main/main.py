...
def train_align(decoder_model, opt_align, args, device, X, Y, Miss_vecs, proto_Num, missindex, final_batch):
    ...
    for epoch in t_progress:
        ...
        for batch_idx, (x, y, miss_vec) in enumerate(train_loader):
            ...
            # 计算epoch平均损失
        epoch_avg_loss = epoch_total_loss / max(batch_count, 1)
        align_losses.append(epoch_avg_loss)

        # 更新进度条显示
        t_progress.set_postfix({'loss': epoch_avg_loss})
...
parser.add_argument('--i_d', type=int, default='2')
parser.add_argument("--protorate", default=0.3, type=float)
parser.add_argument('--missrate', default=0.1, type=float)
...
parser.add_argument('--Batch_Rob', default=128, type=int) #源代码是256
...
def main():
    ...
    fea_end, align_losses = train_align(decoder_model, optimizer_align, args, device, X, Y, Miss_vecs, proto_Num,
    ...
    # 设置刻度字体
    ax.tick_params(axis='both', which='major', labelsize=28, width=3, length=12)  # 主刻度放大加粗
    ax.tick_params(axis='both', which='minor', labelsize=24, width=2.5, length=8)  # 次刻度加粗稍小
    ...
    save_path = f"{save_dir}/alignment_loss_{i_d}_{timestamp}.png"
    fig.savefig(save_path, dpi=300, bbox_inches='tight')  # dpi控制分辨率，bbox_inches保证边界紧凑

    # 同时保存最新版本
    latest_path = f"{save_dir}/alignment_loss_latest.png"
